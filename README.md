...
총 2개의 class로 구성되어있는 데이터셋을 이용해 모델을 완성하고, 분류한 결과에 따라 졸음 운전 여부에 대한 정보를 출력한다.
...
졸음운전은 운전자의 반응속도를 급격히 저하시키며 한순간의 실수가 큰 사고로 이어지는 매우 위험한 요소입니다.<br>
특히 장거리 운전이나 야간 운행에서는 졸음 징후가 미세하게 나타나기 때문에 운전자 스스로 상태를 인지하기 어렵습니다.

이 문제를 해결하기 위하여 얼굴 표정 변화·눈 깜빡임 패턴·고개 움직임과 같은 신호를 실시간으로 분석하여 졸음 징후를 감지하는 딥러닝 기반 모델을 개발했습니다.
