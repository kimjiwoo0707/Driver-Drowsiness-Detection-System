...
### 1) 실험 결과  
본 모델은 운전자 얼굴 이미지를 기반으로 한 졸음 상태 이진 분류 실험에서
높은 분류 정확도와 안정적인 학습 성능을 보였다.
...
### 2) 한계  
이미지 1장당 평균 추론 시간은 0.15초(약 6.7 FPS)로, 모든 프레임을 처리하는 실시간 영상 분석에는 제한이 있다.
다만 프레임 샘플링 기반의 졸음 경고 시스템에는 적용 가능성을 확인하였다.
